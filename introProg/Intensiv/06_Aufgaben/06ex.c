...
{
    float result = (float)value / max; // Ohne (float) ist das Ergebnis immer noch INT
    return result;
}
Canvas swatch_red_green(Canvas c)
{
    for (int i = 0; i < canvas_width(c); i++)
    {
        ...
        {
            canvas_set_r(c, i, j, normalize(i, canvas_width(c) - 1));
            canvas_set_g(c, i, j, normalize(j, canvas_height(c) - 1));
        }
    }
...
Canvas swatch_rgb(Canvas c, ColorChannel channel_x, ColorChannel channel_y)
{
    for (int i = 0; i < canvas_width(c); i++)
    {
        for (int j = 0; j < canvas_height(c); j++)
        {
            canvas_set_channel(c, i, j, channel_x, normalize(i, canvas_width(c) - 1));
            canvas_set_channel(c, i, j, channel_y, normalize(j, canvas_height(c) - 1));
        }
    }
    return c;
}
...
*/
RGB check_collor_at_pixel(Canvas c, int x, int y)
{
    RGB color;
...
Canvas canvas_to_gray(Canvas c)
{
    for (int i = 0; i < canvas_width(c); i++)
    {
        for (int j = 0; j < canvas_height(c); j++)
        {
            RGB color = check_collor_at_pixel(c, i, j);
            color = rgb_to_gray(color);
            canvas_set_rgb_struct(c, i, j, color);
        }
    }
    return c;
}
