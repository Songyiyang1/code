...
def power_iteration(M: np.ndarray, epsilon: float = -1.0) -> (np.ndarray, list):
    ...
    if epsilon == -1.0: epsilon = 10 * np.finfo(float).eps
    ...
    while residual > epsilon:
        ...
        vector_new = np.dot(M, vector) / np.linalg.norm(np.dot(M, vector))
        residual = np.linalg.norm(vector - vector_new)
...
def load_images(path: str, file_ending: str = ".png") -> (list, int, int):
    ...
    images = sorted([image for image in lib.list_directory(path) if image.endswith(file_ending)])
    images = [np.asarray(mpl.image.imread(path + image), dtype=np.float64) for image in images]
